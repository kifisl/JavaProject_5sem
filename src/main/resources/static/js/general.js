...
{
    let greetingField = document.querySelector('.greetingField');

    if(greetingField != null)
    {
        greetingField.innerHTML = 'debug';

        if (await isAuth())
        {
            let user = await getUser();
            greetingField.innerHTML = 'Hi, ' + user.login;
        }
        else
            {
            greetingField.innerHTML = 'Who are you?';
            greetingField.setAttribute('style', 'font-size: 40px !important;')
        }
    }
}